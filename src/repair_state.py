"""
Cache-Aside: proveedor de estado para reglas de reparación.

Separación de responsabilidades: el Validador y el Healing Agent usan este
módulo como CacheProvider (State). No mezclar lógica de "reparación conocida"
con "descubrimiento vía IA"; el Validador orquesta y delega aquí.
"""
import hashlib
...
# Claves para State (Motia Stream)
REPAIR_RULES_GROUP = "sentinel"
REPAIR_RULES_STREAM = "repair_rules"

# TTL por defecto (días); las APIs de terceros pueden cambiar
DEFAULT_TTL_DAYS = 7


def error_signature(error_details: list[dict]) -> str:
    """Clave derivada del tipo de error para cache de reglas (mismo error -> misma regla)."""
    parts = []
...
def apply_rule_idempotent(payload: dict, stored: dict) -> dict:
    """
    Aplica la regla almacenada al payload de forma idempotente.
    No aplica dos veces: si target_field ya existe, no sobrescribe (evita basura).
    """
    out = dict(payload)

    # Formato nuevo: rules + expires_at
...
def _apply_single_rule_idempotent(payload: dict, rule: RepairRule) -> None:
    """Aplica una RepairRule de forma idempotent (modifica payload in-place)."""
    src = rule.source_field
    tgt = rule.target_field
    if src not in payload:
        return
    # Idempotencia: si el destino ya existe y es distinto del origen, no pisar
...
def is_rule_expired(stored: dict) -> bool:
    """True si la regla tiene expires_at y ya pasó la fecha."""
...
def build_stored_rule(rules: list[RepairRule] | list[dict], ttl_days: int = DEFAULT_TTL_DAYS) -> dict:
    """Construye el dict para persistir en State (con expires_at)."""
    now = datetime.utcnow()
...
async def get_repair_rule(ctx: Any, key: str) -> dict | None:
    """Obtiene regla desde State; devuelve None si no existe o está expirada."""
    try:
...
async def set_repair_rule(ctx: Any, key: str, rule: dict, ttl_days: int | None = None) -> None:
    """Guarda regla en State con TTL. Si rule ya tiene expires_at, se respeta; si no, se usa ttl_days."""
